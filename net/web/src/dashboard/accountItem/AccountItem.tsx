        ...
        <div className="handle">
          ...
          {state?.storage > 0 && (
            <span className="storage">
              {state.storage} {state.storageUnit}
            </span>
          )}
        </div>
      ...
      <div className="control">
          ...
          <>
            ...
            {state.disabled && (
              <EnableButton
                ...
                loading={state.statusBusy}
                onClick={() => applyAccountStatus(false)}
              ></EnableButton>
            )}
            {!state.disabled && (
              <DisableButton
                ...
                loading={state.statusBusy}
                onClick={() => applyAccountStatus(true)}
              ></DisableButton>
            )}
            <DeleteButton
              type="text"
        ...
        {state.display !== 'small' && (
          <>
            <Tooltip
              placement="topLeft"
              title={state.strings.accessAccount}
            >
              <ResetButton
                type="text"
                size="large"
            ...
            {state.disabled && (
              <Tooltip
                placement="topLeft"
                title={state.strings.enableAccount}
              >
                <EnableButton
                  type="text"
                  size="large"
      ...
      >
        <AccessLayout>
          ...
          <div className="control">
            <Button
              type="primary"
              onClick={() => actions.setShowAccess(false)}
            >
              {state.strings.ok}
            </Button>
          </div>
