      ...
      incoming.push(
        <div className="ringing-entry">
          <Logo
            ...
            radius={4}
          />
          {ring.name && <div className="ringing-name">{ring.name}</div>}
          {!ring.name && ring.node && (
          ...
          {!ring.name && !ring.node && <div className="ringing-name">{ring.handle}</div>}
          <div
            onClick={() => actions.ignore(ring)}
            className="ringing-ignore"
          >
            <EyeInvisibleOutlined />
          </div>
          <div
            onClick={() => actions.decline(ring)}
            className="ringing-decline"
          >
            <PhoneOutlined />
          </div>
          <div
            onClick={() => actions.accept(ring)}
            className="ringing-accept"
          >
            <PhoneOutlined />
          </div>
        </div>,
      ...
      <SessionWrapper>
            ...
            <div className="center">
              ...
              {state.contact && (
                <div className="reframe base">
                  <Contact
                    close={actions.closeContact}
                    guid={state.contactGuid}
                    listing={state.contactListing}
                  />
                </div>
              )}
              {state.profile && (
              ...
              {state.cards && (
                <div className="reframe">
                  <Cards
                    ...
                    openContact={actions.openContact}
                    openChannel={openConversation}
                    openListing={actions.openListing}
                  />
                  <Drawer
                    ...
                    closable={false}
                    getContainer={false}
                    onClose={actions.closeListing}
                    height={'100%'}
                    width={'80%'}
                    style={{ position: 'absolute', overflow: 'hidden' }}
                  >
                    <Listing
                      closeListing={actions.closeListing}
                      openContact={actions.openContact}
                    />
                  </Drawer>
                </div>
        ...
        {(state.display === 'large' || state.display === 'medium') && (
          <div className="tablet-layout noselect">
            <div className="left">
              <Identity
                openAccount={actions.openProfile}
                openCards={actions.openCards}
                cardUpdated={state.cardUpdated}
              />
              <div className="bottom">
                <Channels
                  open={actions.openConversation}
                  active={{
                    set: state.conversation,
                    card: state.cardId,
                    channel: state.channelId,
            ...
            <div className="right">
              ...
              <Drawer
                ...
                visible={state.profile || state.account}
                zIndex={40}
              >
                {(state.profile || state.account) && <Profile closeProfile={closeAccount} />}
              </Drawer>
            </div>
          </div>
        )}
        ...
        >
          <RingingWrapper>
            <div className="ringing-list">{ringing}</div>
          </RingingWrapper>
        </Modal>
        <Modal
          centered
          visible={state.callStatus}
          footer={null}
          closable={false}
          width={getDim() + 12}
          //@ts-ignore
              ...
              <div className="window">
                ...
                {state.remoteStream && (
                  <video
                    ...
                    autoPlay
                    style={{
                      display: state.remoteVideo ? 'block' : 'none',
                      width: state.fullscreen ? '100%' : getDim(),
                      height: state.fullscreen ? '100%' : getDim(),
                    }}
                    onCompositionEnd={() => console.log('VIDEO COMPLETE')}
                ...
                <div className="calling-options calling-hovered">
                  {state.localVideo && (
                    <div
                      className="calling-option"
                      onClick={actions.disableVideo}
                    >
                      <IoVideocamOutline />
                    </div>
                  )}
