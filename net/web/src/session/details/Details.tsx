    ...
    <DetailsWrapper>
        ...
        <div className="description">
          ...
          <div className="stats">
            {!state.host && (
              <div
                className="subject"
                onClick={actions.setEditSubject}
              >
                {state.sealed && !state.contentKey && <LockFilled style={{ paddingRight: 4 }} />}
                {state.sealed && state.contentKey && <UnlockOutlined style={{ paddingRight: 4 }} />}
                {state.title && <span>{state.title}</span>}
                {!state.title && <span>{state.label}</span>}
            ...
            {!state.host && <div className="host">{state.strings.host}</div>}
            {state.host && <div className="host">{state.strings.guest}</div>}
        ...
        <div className="actions">
          ...
          <div className="controls">
            ...
            {!state.host && !state.sealed && (
              <Tooltip
                placement="top"
                title={state.strings.editMembership}
              >
                <Button
                  className="button"
                  type="primary"
                  ...
                  //@ts-ignore
                  size="medium"
                  onClick={actions.setEditMembers}
                >
      ...
      >
        <EditSubject
          ...
          cancelSubject={actions.clearEditSubject}
          saveSubject={saveSubject}
          setSubject={actions.setSubjectUpdate}
          strings={state.strings}
        />
      </Modal>
      <Modal
        centered
        visible={state.showEditMembers}
        closable={false}
        footer={null}
        bodyStyle={{ borderRadius: 8, padding: 16, ...state.menuStyle }}
      >
        <EditMembers
          ...
          onClose={actions.clearEditMembers}
          strings={state.strings}
        />
      </Modal>
    </DetailsWrapper>
