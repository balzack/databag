...
export function AccountAccess() {
  ...
  const logout = () => {
    modal.confirm({
      ...
      icon: <LogoutOutlined />,
      content: (
        <LogoutContent onClick={(e) => e.stopPropagation()}>
          <span className="logoutMode">{state.strings.allDevices}</span>
          <Switch
            onChange={(e) => (all.current = e)}
            size="small"
          />
        </LogoutContent>
      ),
  ...
  return (
    <AccountAccessWrapper>
          ...
          <div className="switch">
            <div className="control">
              <Switch
                ...
                onChange={(enable) => saveSearchable(enable)}
              />
            </div>
      ...
      <div className="account">
        ...
        <div className="controls">
          <div className="option">
            ...
            >
              <Radio
                style={{ borderRadius: 3, paddingLeft: 6, ...state.menuStyle }}
                value={'12h'}
              >
                {state.strings.timeUs}
              </Radio>
              <Radio
                style={{ borderRadius: 3, paddingLeft: 6, ...state.menuStyle }}
                value={'24h'}
              >
                {state.strings.timeEu}
              </Radio>
            </Radio.Group>
          </div>
          <div className="option">
            ...
            >
              <Radio
                ...
                value={'mm/dd'}
              >
                {state.strings.dateUs}
              </Radio>
              <Radio
                style={{ borderRadius: 3, paddingLeft: 6, ...state.menuStyle }}
                value={'dd/mm'}
              >
                {state.strings.dateEu}
              </Radio>
            </Radio.Group>
          </div>
          <div className="option">
      ...
      <Modal
        ...
        footer={null}
        onCancel={actions.clearEditSeal}
        bodyStyle={{ borderRadius: 8, padding: 16, ...state.menuStyle }}
      >
        <SealModal>
          ...
          <div className="switch">
            <Switch
              ...
              onChange={(enable) => actions.enableSeal(enable)}
            />
          ...
          {(state.sealMode === 'updating' || state.sealMode === 'enabling') && (
            <div className="sealChange">
              <Input.Password
                ...
                spellCheck="false"
                onChange={(e) => actions.setSealPassword(e.target.value)}
          ...
          {(state.sealMode === 'updating' || state.sealMode === 'enabling') && (
            <div className="sealChange">
              <Input.Password
                ...
                spellCheck="false"
                onChange={(e) => actions.setSealConfirm(e.target.value)}
                autocomplete="new-password"
                prefix={<LockOutlined />}
              />
            </div>
          ...
          {state.sealMode === 'disabling' && (
            <div className="sealChange">
              <Input
                ...
                spellCheck="false"
                onChange={(e) => actions.setSealDelete(e.target.value)}
          ...
          {state.sealMode === 'enabled' && (
            <div
              className="sealChange"
              onClick={() => actions.updateSeal()}
            >
              <Input.Password
          ...
          {state.sealMode === 'unlocking' && (
            <div className="sealChange">
              <Input.Password
                ...
                spellCheck="false"
                onChange={(e) => actions.setSealUnlock(e.target.value)}
                prefix={<LockOutlined />}
              />
            </div>
          )}
          <div className="controls">
            ...
            {state.sealMode === 'unlocking' && (
              <Button
                ...
                type="primary"
                onClick={saveSeal}
                className={actions.canSaveSeal() ? 'saveEnabled' : 'saveDisabled'}
                disabled={!actions.canSaveSeal()}
                loading={state.busy}
              >
      ...
      >
        <LoginModal>
          ...
          <Input
            ...
            spellCheck="false"
            onChange={(e) => actions.setEditHandle(e.target.value)}
            defaultValue={state.editHandle}
          ...
          <Input.Password
            ...
            placeholder={state.strings.newPassword}
            spellCheck="false"
            onChange={(e) => actions.setEditPassword(e.target.value)}
            autocomplete="new-password"
            prefix={<LockOutlined />}
          />
          ...
          <Input.Password
            ...
            placeholder={state.strings.confirmPassword}
            spellCheck="false"
            onChange={(e) => actions.setEditConfirm(e.target.value)}
            autocomplete="new-password"
            prefix={<LockOutlined />}
          />
          <div className="controls">
            <Button
              key="back"
              onClick={actions.clearEditLogin}
            >
              {state.strings.cancel}
            </Button>
            <Button
              key="save"
              type="primary"
              className={actions.canSaveLogin() ? 'saveEnabled' : 'saveDisabled'}
              onClick={saveLogin}
              disabled={!actions.canSaveLogin()}
              loading={state.busy}
            >
              {state.strings.save}
            </Button>
