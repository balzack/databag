  ...
  return (
    <ChannelsWrapper>
      <div className="search">
        ...
        {state.display === 'small' && (
          <div className="inline">
            <Button
              type="primary"
              disabled={!state.allowAdd}
              icon={<CommentOutlined />}
              onClick={actions.setShowAdd}
            >
              {state.strings.new}
            </Button>
          </div>
          ...
          <List
            ...
            renderItem={(item) => (
              <ChannelItem
                item={item}
                openChannel={open}
                tip={state.strings.newMessage}
                active={active.card === item.cardId && active.channel === item.channelId}
              />
            )}
          />
        )}
        {state.channels.length === 0 && <div className="empty">{state.strings.noTopics}</div>}
      </div>
      {state.display !== 'small' && (
        <div className="bar">
          <Tooltip
            placement="right"
            title={state.allowAdd ? '' : state.strings.unsetSealing}
          >
            <Button
              className={state.allowAdd ? 'addEnabled' : 'addDisabled'}
              type="primary"
              disabled={!state.allowAdd}
              icon={<CommentOutlined />}
              onClick={actions.setShowAdd}
            >
              {state.strings.newTopic}
            </Button>
          </Tooltip>
        </div>
      )}
      <Modal
        ...
        closable={false}
        centered
        visible={state.showAdd && state.allowAdd}
        footer={null}
        destroyOnClose={true}
      ...
      >
        <AddChannel
          added={added}
          cancelled={actions.clearShowAdd}
        />
      </Modal>
    </ChannelsWrapper>
  );
