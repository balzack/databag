...
func NotifyChannelRevision(card *store.Card, revision int64) error {
	...
	err := store.DB.Transaction(func(tx *gorm.DB) error {
		...
		if res := tx.Model(&store.CardSlot{}).Where("id = ?", card.CardSlot.ID).Update("revision", act.CardRevision+1).Error; res != nil {
			return res
		}
		if res := tx.Model(&store.Account{}).Where("id = ?", act.ID).Update("card_revision", act.CardRevision+1).Error; res != nil {
			return res
		}
		return nil
	})
