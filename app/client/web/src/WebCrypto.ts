...
export class WebCrypto implements Crypto {
  ...
  public pbkdfSalt(): { saltHex: string } {
  ...
  public aesKey(): { aesKeyHex: string } {
  ...
  public aesEncrypt(data: string, ivHex: string, aesKeyHex: string): { encryptedDataB64: string } {
  ...
  public aesDecrypt(encryptedDataB64: string, ivHex: string, aesKeyHex: string): { data: string } {
    const iv = CryptoJS.enc.Hex.parse(ivHex);
    const key = CryptoJS.enc.Hex.parse(aesKeyHex);
  ...
  public rsaEncrypt(data: string, publicKeyB64: string): { encryptedDataB64: string } {
  ...
  public rsaDecrypt(encryptedDataB64: string, privateKeyB64: string): { data: string } {
    const crypto = new JSEncrypt();
